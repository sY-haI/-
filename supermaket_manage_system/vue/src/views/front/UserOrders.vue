<template>
  <div class="front-container" style="width: 80%">
    ...
    <div style="margin-bottom: 10px;">
      ...
      <el-button style="height: 40px" type="primary" @click="load">查询</el-button>
    </div>

    <div class="card" style="margin-bottom: 5px">
      <el-table :data="data.tableData" stripe :cell-style="{'backgroundColor': '#dddddd'}" default-expand-all>
          ...
          <template #default="props">
            <div style="padding: 10px">
              ...
              <div style="text-align: right; margin-top: 10px; padding: 10px; background-color: #f5f5f5; border-radius: 5px">
                ...
                <b style="color: red; font-size: 18px">{{ props.row.actualPrice || props.row.total }}元</b>
        ...
        <el-table-column prop="actualPrice" label="实付金额" width="120">
          <template #default="scope">
            <b style="color: red; font-size: 16px">{{ scope.row.actualPrice || scope.row.total }}元</b>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="deliver" label="配送信息" width="200" show-overflow-tooltip></el-table-column>
        <el-table-column label="订单操作" align="center" width="120">
          <template #default="scope">
            ...
            <el-button @click="cancle(scope.row)" type="danger" v-if="scope.row.status === '待接单'">取消</el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog title="评价信息" width="30%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
      <el-form ref="formRef" :model="data.form" :rules="data.rules" label-width="80px" style="padding-right: 30px; padding-top: 20px">
        ...
        <el-form-item label="内容" prop="content">
          <el-input type="textarea" :row="3" v-model="data.form.content" autocomplete="off" placeholder="请输入评价内容"/>
        </el-form-item>
      </el-form>
      <template #footer>
...
// 取消订单
const cancle = (row) =>{
  ElMessageBox.confirm('您确认取消订单了吗？', '二次确认', { type: 'warning' }).then(res => {
    data.form = row
...
// 确认收货
const done = (row) =>{
  ElMessageBox.confirm('您确认收到货物了吗？', '二次确认', { type: 'warning' }).then(res => {
    data.form = row
