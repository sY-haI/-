...
const getCurrentUser = () => {
  try {
    // 从localStorage获取用户信息，key是 'system-user'
...
// 判断是否是当前用户
const isCurrentUser = (rowId) => {
  return String(rowId) === String(currentUserId.value)
}
...
const handleDelete = (row) => {
  ...
  // 使用 ElMessageBox 弹窗确认
  ElMessageBox.confirm(
      `确定要删除管理员 "${row.name}" 吗？删除后数据无法恢复！`,
      '删除确认',
      {
